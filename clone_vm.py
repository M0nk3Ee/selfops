...
The majority of this has been shamelessly stolen from https://gist.github.com/snobear/8788977

Makes extensive use of the VMware python bindings: http://vmware.github.io/pyvmomi-community-samples/
...
./clone.py --hostname test01 --template centos65 --ips 10.3.0.120 --cpus 2 --mem 4
...
def WaitTask(task, actionName='job', hideResult=False):
    ...
    while task.info.state == vim.TaskInfo.State.running:
       ...
       if isinstance(task.info.progress, int): 
          print "Progress : " + str(task.info.progress) + "%"
          time.sleep(3)
    
    #if task.info.state == vim.TaskInfo.State.success:
    if task.info.state == "success":
       print "Progress : 100%"
       #task.info.result = "success"
...
def clone(deploy_settings, ip_settings):
    ...
    print " "
    print "Settings about to be used for deploy..." 
    print "VSphere Server: %s" % deploy_settings["vserver"]
    print "VSphere username: %s" % deploy_settings["username"]
    print "VM hostname: %s" % deploy_settings["new_vm_name"]
    print "VM Datacentre: %s" % ip_settings[0]["datacenter_name"]
    print "VM Cluster: %s" % ip_settings[0]["cluster_name"]
    print "VM Template: %s" % deploy_settings["template_name"]
    ...
    for key, ip in enumerate(ip_settings):
        ...
        print "VM IP: %s" % str(ip_settings[key]["ip"])
        guest_map.adapter.subnetMask = str(ip_settings[key]["subnet_mask"])
        print "VM Subnetmask: %s" % str(ip_settings[key]["subnet_mask"])
    ...
    print "VM vCPUs: %s" % deploy_settings['cpus']
    vmconf.memoryMB = deploy_settings['mem']
    print "VM Memory: %s" % deploy_settings['mem']
    ...
    print " " 
    if query_yes_no("Proceed with deployment?"):
        print " " 
        # fire the clone task
...
def deploy(deploy_vm_hostname, deploy_vm_template, deploy_vm_ip, deploy_vm_cpus, deploy_vm_mem):
    deploy_settings["new_vm_name"] = deploy_vm_hostname.lower()
    deploy_settings['cpus'] = deploy_vm_cpus 
    deploy_settings['mem'] = deploy_vm_mem * 1024
    ...
    '''
    '''The following line was added until we start parsing args properly in main python file'''
    temp_ip_list = []
    temp_ip_list.append(deploy_vm_ip)
    for key, ip_string in enumerate(temp_ip_list):
    ...
    deploy_settings['template_name'] = deploy_vm_template 

    # clone template to a new VM with our specified settings
    result = clone(deploy_settings, ip_settings)
    return result
